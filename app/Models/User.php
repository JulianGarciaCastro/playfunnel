...
class User extends Authenticatable implements MustVerifyEmail, FilamentUser
{
    ...
    public function canAccessFilament(): bool
    {
        ...
        return $this->hasPermissionSafely('panel.access');
    }
    ...
    public function hasPermissionSafely(string $permission): bool
    {
        try {
            ...
            Log::warning('RBAC permission check failed', [
                'user_id' => $this->id,
    ...
    {
        if (! $this->hasPermissionSafely('users.impersonate')) {
            return false;
        }
    ...
    public function isCurrentSubscriptionCanceled()
    {
        ...
        if (!$planSubscription) {
            return false; // no hay suscripción => no está cancelada
        }
    ...
    public function getNextBillingDate()
    {
        ...
        if ($this->isCurrentSubscriptionCanceled()) return "";

        if (empty($planSubscription->invoicenum)) return "";

        try {
            $stripe = new \Stripe\StripeClient(env('STRIPE_SECRET'));
            $subscription = $stripe->subscriptions->retrieve($planSubscription->invoicenum, []);
            $next = Carbon::createFromTimestamp($subscription->current_period_end);
            return $next->format('d-m-Y');
        } catch (\Exception $e) {
            Log::error("Stripe getNextBillingDate() user {$this->id}: ".$e->getMessage());
            return "Stripe: no existe"; // o "" si prefieres silencioso
        }
    }
